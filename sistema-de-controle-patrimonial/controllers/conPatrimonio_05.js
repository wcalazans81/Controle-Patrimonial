...
router.post('/cadastrar_05', (req, res) => {
    let { andar_05, Npatrimonio_05, empresa_05, nome_05, setor_05, descricao_05, destino_05 } = req.body;

    // Verificar se os campos são strings antes de usar .toUpperCase()
    empresa_05 = typeof empresa_05 === 'string' ? empresa_05.toUpperCase() : '';
    nome_05 = typeof nome_05 === 'string' ? nome_05.toUpperCase() : '';
    setor_05 = typeof setor_05 === 'string' ? setor_05.toUpperCase() : '';
    descricao_05 = typeof descricao_05 === 'string' ? descricao_05.toUpperCase() : '';
    destino_05 = typeof destino_05 === 'string' ? destino_05.toUpperCase() : '';
    ...
    if (!empresasValidas.includes(empresa_05)) {
    ...
    const sql = `
        INSERT INTO patrimonio_05 (
            andar_05, Npatrimonio_05, empresa_05, nome_05, setor_05, data_05, descricao_05, destino_05
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?)
    `;

    conexao.query(sql, [andar_05, Npatrimonio_05, empresa_05, nome_05, setor_05, data, descricao_05, destino_05], (erro) => {
        if (erro) {
        ...
        conexao.query(sql2, [andar_05, Npatrimonio_05, empresa_05, nome_05, setor_05, data, descricao_05, destino_05], (erro2) => {
            if (erro2) {
...
router.get('/excluir_05/:id', (req, res) => {
    const id = req.params.id;

    // Busca os dados do patrimônio a ser excluído
    const sqlSelect = 'SELECT * FROM patrimonio_05 WHERE id = ?';
    conexao.query(sqlSelect, [id], (erroSelect, resultadoSelect) => {
        ...
        conexao.query(sqlCheck, [item.Npatrimonio_05], (erroCheck, resultadoCheck) => {
            if (erroCheck) {
            ...
            const fazerDeletePatrimonio = () => {
                const sqlDelete = 'DELETE FROM patrimonio_05 WHERE id = ?';
                conexao.query(sqlDelete, [id], (erroDelete) => {
                    ...
                    return res.redirect('/patrimonio_05');
                });
            };
            ...
            if (resultadoCheck.length > 0) {
                ...
                conexao.query(sqlDeleteRelat, [item.Npatrimonio_05], (erroDeleteRelat) => {
                    if (erroDeleteRelat) {
                ...
                // Caso não exista correspondência, só exclui da patrimonio_05
                fazerDeletePatrimonio();
            }
        });
    });
