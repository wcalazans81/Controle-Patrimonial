...
document.addEventListener("DOMContentLoaded", function () {
    ...
    const btnSalvar00 = document.getElementById("salvar_00");
    if (btnSalvar00) {
        btnSalvar00.addEventListener("click", function (event) {
            event.preventDefault();

            let andar = document.querySelector("input[name='andar_00']").value.trim().toUpperCase();
            let patrimonio = document.querySelector("input[name='patrimonio_00']").value.trim().toUpperCase();
            let empresa = document.querySelector("input[name='empresa_00']").value.trim().toUpperCase();
            let nome = document.querySelector("input[name='nome_00']").value.trim().toUpperCase();
            let setor = document.querySelector("input[name='setor_00']").value.trim().toUpperCase();
            let descricao = document.querySelector("input[name='descricao_00']").value.trim().toUpperCase();
            let destino = document.querySelector("input[name='destino_00']:checked");
            ...
            let dados = {
                andar_00: andar,
                Npatrimonio_00: patrimonio,
                empresa_00: empresa,
                nome_00: nome,
                setor_00: setor,
                descricao_00: descricao,
                destino_00: destino
            };
...
if (patrimonioDuplicado) {
    ...
    const btnMostrarBusca = document.getElementById("btnMostrarBusca_00");
    const campoBusca = document.getElementById("campoBusca_00");
    const btnBuscar = document.getElementById("btnBuscarPatrimonio_00");
...
const btnSalvarPdf = document.getElementById("btnSalvarPdf_00");
if (btnSalvarPdf) {
