document.addEventListener("DOMContentLoaded", function () { 
    ...
    // === EVENTO BOTÃO "salvar_12" - Envia dados via fetch ===
    const btnSalvar00 = document.getElementById("salvar_12");
    if (btnSalvar00) {
        btnSalvar00.addEventListener("click", function (event) {
            event.preventDefault();

            let andar = document.querySelector("input[name='andar_12']").value.trim().toUpperCase();
            let patrimonio = document.querySelector("input[name='patrimonio_12']").value.trim().toUpperCase();
            let empresa = document.querySelector("input[name='empresa_12']").value.trim().toUpperCase();
            let nome = document.querySelector("input[name='nome_12']").value.trim().toUpperCase();
            let setor = document.querySelector("input[name='setor_12']").value.trim().toUpperCase();
            let descricao = document.querySelector("input[name='descricao_12']").value.trim().toUpperCase();
            let destino = document.querySelector("input[name='destino_12']:checked");
            ...
            let dados = {
                andar_12: andar,
                Npatrimonio_12: patrimonio,
                empresa_12: empresa,
                nome_12: nome,
                setor_12: setor,
                descricao_12: descricao,
                destino_12: destino
            };
    ...
    const btnMostrarBusca = document.getElementById("btnMostrarBusca_12");
    const campoBusca = document.getElementById("campoBusca_12");
    const btnBuscar = document.getElementById("btnBuscarPatrimonio_12");
    ...
    cards.forEach((card, index) => {
        ...
        if (destinoIndefinido && mesAtual >= 10 && !jaClicadoEsteAno) {
            card.style.border = "12px solid red";
    ...
    const btnSalvarPdf = document.getElementById("btnSalvarPdf_12");
    if (btnSalvarPdf) {
