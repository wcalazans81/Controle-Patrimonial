document.addEventListener("DOMContentLoaded", function () { 
    ...
    // === EVENTO BOTÃO "salvar_11" - Envia dados via fetch ===
    const btnSalvar00 = document.getElementById("salvar_11");
    if (btnSalvar00) {
        btnSalvar00.addEventListener("click", function (event) {
            event.preventDefault();

            let andar = document.querySelector("input[name='andar_11']").value.trim().toUpperCase();
            let patrimonio = document.querySelector("input[name='patrimonio_11']").value.trim().toUpperCase();
            let empresa = document.querySelector("input[name='empresa_11']").value.trim().toUpperCase();
            let nome = document.querySelector("input[name='nome_11']").value.trim().toUpperCase();
            let setor = document.querySelector("input[name='setor_11']").value.trim().toUpperCase();
            let descricao = document.querySelector("input[name='descricao_11']").value.trim().toUpperCase();
            let destino = document.querySelector("input[name='destino_11']:checked");
            ...
            let dados = {
                andar_11: andar,
                Npatrimonio_11: patrimonio,
                empresa_11: empresa,
                nome_11: nome,
                setor_11: setor,
                descricao_11: descricao,
                destino_11: destino
            };
    ...
    const btnMostrarBusca = document.getElementById("btnMostrarBusca_11");
    const campoBusca = document.getElementById("campoBusca_11");
    const btnBuscar = document.getElementById("btnBuscarPatrimonio_11");
    ...
    cards.forEach((card, index) => {
        ...
        if (destinoIndefinido && mesAtual >= 10 && !jaClicadoEsteAno) {
            card.style.border = "11px solid red";
    ...
    const btnSalvarPdf = document.getElementById("btnSalvarPdf_11");
    if (btnSalvarPdf) {
        btnSalvarPdf.addEventListener("click", () => {
            ...
            let tabelaHTML = `
                <h1>${titulo}</h1>
                <h3>${data}</h3>
                <table border="1" cellpadding="11" cellspacing="0" style="width:100%; border-collapse:collapse">
                    <thead>
                        <tr>
                            <th>Conteúdo</th>
                        </tr>
