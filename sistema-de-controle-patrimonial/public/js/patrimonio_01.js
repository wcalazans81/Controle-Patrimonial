document.addEventListener("DOMContentLoaded", function () {
    ...
    // === EVENTO BOTÃO "salvar_01" - Envia dados via fetch ===
    const btnSalvar00 = document.getElementById("salvar_01");
    if (btnSalvar00) {
        btnSalvar00.addEventListener("click", function (event) {
            event.preventDefault();

            let andar = document.querySelector("input[name='andar_01']").value.trim().toUpperCase();
            let patrimonio = document.querySelector("input[name='patrimonio_01']").value.trim().toUpperCase();
            let empresa = document.querySelector("input[name='empresa_01']").value.trim().toUpperCase();
            let nome = document.querySelector("input[name='nome_01']").value.trim().toUpperCase();
            let setor = document.querySelector("input[name='setor_01']").value.trim().toUpperCase();
            let descricao = document.querySelector("input[name='descricao_01']").value.trim().toUpperCase();
            let destino = document.querySelector("input[name='destino_01']:checked");
            ...
            let dados = {
                andar_01: andar,
                Npatrimonio_01: patrimonio,
                empresa_01: empresa,
                nome_01: nome,
                setor_01: setor,
                descricao_01: descricao,
                destino_01: destino
            };

    // Verifica se o patrimônio já existe nos cards
...
if (patrimonioDuplicado) {
    ...
    const btnMostrarBusca = document.getElementById("btnMostrarBusca_01");
    const campoBusca = document.getElementById("campoBusca_01");
    const btnBuscar = document.getElementById("btnBuscarPatrimonio_01");
   ...
   // === DESTACAR "USO INDEFINIDO" COM BORDA VERMELHA SE MÊS ATUAL >= OUTUBRO, UMA VEZ POR ANO ===
    ...
    cards.forEach((card, index) => {
        ...
        // Verifica se o destino é "USO INDEFINIDO"
        const destinoIndefinido = destinoTexto.includes("USO INDEFINIDO");

        // Chave única por card (você pode usar ID se houver no HTML)
        ...
        if (destinoIndefinido && mesAtual >= 10 && !jaClicadoEsteAno) {
            ...
            // Clicar remove a borda e salva no localStorage para não reaparecer neste ano
...
const btnSalvarPdf = document.getElementById("btnSalvarPdf_01");
if (btnSalvarPdf) {
