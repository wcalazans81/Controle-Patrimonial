document.addEventListener("DOMContentLoaded", function () { 
    ...
    // === EVENTO BOTÃO "salvar_10" - Envia dados via fetch ===
    const btnSalvar00 = document.getElementById("salvar_10");
    if (btnSalvar00) {
        btnSalvar00.addEventListener("click", function (event) {
            event.preventDefault();

            let andar = document.querySelector("input[name='andar_10']").value.trim().toUpperCase();
            let patrimonio = document.querySelector("input[name='patrimonio_10']").value.trim().toUpperCase();
            let empresa = document.querySelector("input[name='empresa_10']").value.trim().toUpperCase();
            let nome = document.querySelector("input[name='nome_10']").value.trim().toUpperCase();
            let setor = document.querySelector("input[name='setor_10']").value.trim().toUpperCase();
            let descricao = document.querySelector("input[name='descricao_10']").value.trim().toUpperCase();
            let destino = document.querySelector("input[name='destino_10']:checked");
            ...
            let dados = {
                andar_10: andar,
                Npatrimonio_10: patrimonio,
                empresa_10: empresa,
                nome_10: nome,
                setor_10: setor,
                descricao_10: descricao,
                destino_10: destino
            };
    ...
    const btnMostrarBusca = document.getElementById("btnMostrarBusca_10");
    const campoBusca = document.getElementById("campoBusca_10");
    const btnBuscar = document.getElementById("btnBuscarPatrimonio_10");
    ...
    const btnSalvarPdf = document.getElementById("btnSalvarPdf_10");
    if (btnSalvarPdf) {
