document.addEventListener("DOMContentLoaded", function () { 
    ...
    // === EVENTO BOTÃO "salvar_05" - Envia dados via fetch ===
    const btnSalvar00 = document.getElementById("salvar_05");
    if (btnSalvar00) {
        btnSalvar00.addEventListener("click", function (event) {
            event.preventDefault();

            let andar = document.querySelector("input[name='andar_05']").value.trim().toUpperCase();
            let patrimonio = document.querySelector("input[name='patrimonio_05']").value.trim().toUpperCase();
            let empresa = document.querySelector("input[name='empresa_05']").value.trim().toUpperCase();
            let nome = document.querySelector("input[name='nome_05']").value.trim().toUpperCase();
            let setor = document.querySelector("input[name='setor_05']").value.trim().toUpperCase();
            let descricao = document.querySelector("input[name='descricao_05']").value.trim().toUpperCase();
            let destino = document.querySelector("input[name='destino_05']:checked");
            ...
            let dados = {
                andar_05: andar,
                Npatrimonio_05: patrimonio,
                empresa_05: empresa,
                nome_05: nome,
                setor_05: setor,
                descricao_05: descricao,
                destino_05: destino
            };
    ...
    const btnMostrarBusca = document.getElementById("btnMostrarBusca_05");
    const campoBusca = document.getElementById("campoBusca_05");
    const btnBuscar = document.getElementById("btnBuscarPatrimonio_05");
    ...
    const btnSalvarPdf = document.getElementById("btnSalvarPdf_05");
    if (btnSalvarPdf) {
